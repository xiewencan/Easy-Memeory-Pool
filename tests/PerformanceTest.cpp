...
public:
    ...
    static void warmup() 
    {
        ...
        {
            EasyMemoryPool::deallocate(ptr, size);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation() 
    {
        ...
        // 测试内存池
        {
            ...
            for (void* ptr : ptrs) 
            {
                EasyMemoryPool::deallocate(ptr, SMALL_SIZE);
            }
    ...
    static void testMultiThreaded() 
    {
        ...
        auto threadFunc = [](bool useMemPool) 
        {
            ...
            {
                size_t size = dis(gen);
                void* ptr = useMemPool ? EasyMemoryPool::allocate(size) 
                                     : new char[size];
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 100 == 0 && !ptrs.empty()) 
                {
                    ...
                    {
                        EasyMemoryPool::deallocate(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
